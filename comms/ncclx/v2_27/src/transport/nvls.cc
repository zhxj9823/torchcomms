...
static ncclResult_t nvlsAllocateMem(struct ncclComm* comm, const CUmemAccessDesc* desc, size_t size, CUmemGenericAllocationHandle* ucHandle, CUmemGenericAllocationHandle* mcHandle, void** ucptr, void** mcptr, size_t* ucsizePtr, size_t* mcsizePtr, CUmulticastGranularity_flags mcOption = CU_MULTICAST_GRANULARITY_RECOMMENDED) {
...
ncclResult_t ncclNvlsBufferSetup(struct ncclComm* comm) {
  ...
  NCCLCHECKGOTO(nvlsAllocateMem(comm, &resources->accessDesc, nvlsTotalSize, &resources->ucBuffHandle, &resources->mcBuffHandle, (void**)&resources->ucBuff, (void**)&resources->mcBuff, &resources->buffUCSize, &resources->buffMCSize, ncclx::memory::adjustCuMulticastGran()), res, fail);
...
ncclResult_t ncclNvlsSetup(struct ncclComm* comm, struct ncclComm* parent) {
  ...
  comm->nvlsChunkSize = NCCL_NVLS_CHUNKSIZE;
  if (nvlsShare) {
