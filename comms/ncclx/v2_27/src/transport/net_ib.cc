...
static ncclResult_t ncclIbStatsCheckFatalCount(struct ncclIbStats* stat, const char* funcName) {
  if (NCCL_IB_RETURN_ASYNC_EVENTS && __atomic_load_n(&stat->fatalErrorCount, __ATOMIC_RELAXED)) {
...
static ncclResult_t ncclIbGetGidIndex(struct ibv_context *context, uint8_t portNum, struct ibv_port_attr* portAttr, int *gidIndex) {
  ...
  if (portAttr->link_layer == IBV_LINK_LAYER_INFINIBAND) {
    union ibv_gid gid;
    int routableGidIndex = NCCL_IB_ROUTABLE_FLID_GID_INDEX;
...
ncclResult_t ncclIbMakeVDeviceInternal(int* d, ncclNetVDeviceProps_t* props) {
  if (NCCL_IB_MERGE_NICS == 0 && props->ndevs > 1) {
...
ncclResult_t ncclIbInit(ncclDebugLogger_t logFunction, ncclProfilerCallback_t profFunction) {
  ...
  if (ncclNIbDevs == -1) {
    pthread_mutex_lock(&ncclIbLock);
    enum ibv_fork_status forkStatus;
    NCCLCHECK(wrap_ibv_is_fork_initialized(&forkStatus));
    // forkStatus will be IBV_FORK_UNNEEDED if eager copy on fork is supported,
    // in which case, we dont need to call ibv_fork_init.
    if (forkStatus == IBV_FORK_DISABLED) {
      WARN("NET/IB : Calling ibv_fork_init. This may result in undefined behavior when non-page aligned buffers are used with fork.");
      wrap_ibv_fork_init();
    }
    if (ncclNIbDevs == -1) {
      int nIpIfs = 0;
      ...
      std::string userIbEnvOrigStr = NCCL_IB_HCA_PREFIX + folly::join(",", NCCL_IB_HCA);
      const char* userIbEnv = userIbEnvOrigStr.c_str();
      ...
      for (int d=0; d<nIbDevs && ncclNIbDevs<MAX_IB_DEVS; d++) {
        ...
        if (wrap_mlx5dv_is_supported(devices[d])) {
          ...
          if((ncclMlx5dvDmaBufCapable(context)) && (wrap_mlx5dv_get_data_direct_sysfs_path(context, dataDirectDevicePath + 4, PATH_MAX - 4) == ncclSuccess)) {
            ...
            if(NCCL_IB_DATA_DIRECT == 1) { dataDirectSupported = 1; skipNetDevForDataDirect = 1; }
            if(NCCL_IB_DATA_DIRECT == 2) { dataDirectSupported = 1; skipNetDevForDataDirect = 0; }
          }
        }
        int nPorts = 0;
...
struct alignas(32) ncclIbNetCommBase {
  ...
  struct ncclIbStats stats;
  // Track logged collectives to prevent duplicate logging
  bool loggedCollectives[ncclNumFuncs];
};
...
ncclResult_t ncclIbInitCommDevBase(int ibDevN, struct ncclIbNetCommDevBase* base, void* cq_context) {
  ...
  NCCLCHECK(wrap_ibv_create_cq(&base->cq, ibDev->context, 2*MAX_REQUESTS*NCCL_IB_QPS_PER_CONNECTION, cq_context, NULL, 0));
...
ncclResult_t ncclIbRtrQp(struct ibv_qp* qp, struct ncclIbGidInfo* sGidInfo, uint32_t dest_qp_num, struct ncclIbDevInfo* info, bool fifoTc, int tc, int sl) {
  ...
  if (info->link_layer == IBV_LINK_LAYER_ETHERNET) {
    ...
    qpAttr.ah_attr.grh.traffic_class = fifoTc && NCCL_IB_FIFO_TC != -1 ? NCCL_IB_TC : tc;
  } else {
...
ncclResult_t ncclIbConnect(int dev, ncclNetCommConfig_t* config, void* opaqueHandle, void** sendComm, ncclNetDeviceHandle_t** /*sendDevComm*/) {
  ...
  *sendComm = NULL;
  ...
  // Initialize loggedCollectives array
  for (int i = 0; i < ncclNumFuncs; i++) {
    comm->base.loggedCollectives[i] = false;
  }
...
ib_recv_dev_list:
  ...
  localNqps  = NCCL_IB_QPS_PER_CONNECTION * comm->base.vProps.ndevs; // We must have at least 1 qp per-device
  remoteNqps = NCCL_IB_QPS_PER_CONNECTION * remoteVProps.ndevs;
  ...
  meta.sl = (NCCL_IB_SL != -1) ? NCCL_IB_SL : (config && config->trafficClass != NCCL_NET_TRAFFIC_CLASS_UNDEF) ? config->trafficClass : NCCL_IB_SL_DEFAULT;
  meta.tc = (NCCL_IB_TC != -1) ? NCCL_IB_TC : (config && config->trafficClass != NCCL_NET_TRAFFIC_CLASS_UNDEF) ? config->trafficClass : NCCL_IB_TC_DEFAULT;
...
ib_connect:
  ...
  for (int q = 0; q < comm->base.nqps; q++) {
    ...
    INFO(NCCL_REG, "[IB_CONN_CONNECT],LocalDev,%s,lqpn,%u,RemoteDev,%s,RemoteQP,%u",
          ibDev->devName, qp->qp_num,
          remMeta.devName, remQpInfo->qpn);
  }
...
ncclResult_t ncclIbCheckVProps(ncclNetVDeviceProps_t* vProps1, ncclNetVDeviceProps_t* vProps2) {
  ...
  if (NCCL_IB_WARN_RAIL_LOCAL && outVProps.ndevs < maxVProps->ndevs) {
    char local[128];
    int cursor = 1;
...
ncclResult_t ncclIbAccept(void* listenComm, void** recvComm, ncclNetDeviceHandle_t** /*recvDevComm*/) {
  ...
  *recvComm = NULL;
  ...
  // Initialize loggedCollectives array
  for (int i = 0; i < ncclNumFuncs; i++) {
    rComm->base.loggedCollectives[i] = false;
  }
...
ib_recv_dev_list:
  ...
  int localNqps, remoteNqps;
  localNqps  = NCCL_IB_QPS_PER_CONNECTION * rComm->base.vProps.ndevs; // We must have at least 1 qp per-device
  remoteNqps = NCCL_IB_QPS_PER_CONNECTION * remoteVProps.ndevs;
...
ib_recv:
  ...
  for (int q = 0; q < rComm->base.nqps; q++) {
    ...
    INFO(NCCL_REG, "[IB_CONN_ACCEPT],LocalDev,%s,lqpn,%u,RemoteDev,%s,RemoteQP,%u",
         ibDev->devName, qp->qp->qp_num,
         remMeta.devName, remMeta.qpInfo[q].qpn);
  }
...
ncclResult_t ncclIbMultiSend(struct ncclIbSendComm* comm, int slot) {
  ...
  const int align = 128;
  int nqps = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.nDataQps;
...
ncclResult_t ncclIbIsend(void* sendComm, void* data, size_t size, int tag, void* mhandle, void* phandle, void** request) {
  ...
  for (int r=0; r<nreqs; r++) {
    ...
    // Populate events
    int nEvents = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.nDataQps;
    int qpIndex = comm->base.qpIndex;
    // Count down
    while (nEvents > 0) {
      ncclIbQp* qp = comm->base.qps + qpIndex;
...
ncclResult_t ncclIbIrecv(void* recvComm, int n, void** data, size_t* sizes, int* tags, void** mhandles, void** phandles, void** request) {
  ...
  const int nqps = NCCL_IB_SPLIT_DATA_ON_QPS ? comm->base.nqps : comm->base.nDataQps;
...
ncclResult_t ncclIbLogCollective(void* comm, uint8_t collOp, uint64_t commHash, bool isSend) {
  // Determine comm type and access pattern
  struct ncclIbNetCommBase* base;
  const char* logPrefix;

  if (isSend) {
    base = &((struct ncclIbSendComm*)comm)->base;
    logPrefix = "[COLLECTIVE_QP_SEND]";

  } else {
    base = &((struct ncclIbRecvComm*)comm)->base;
    logPrefix = "[COLLECTIVE_QP_RECV]";
  }

  // Common logging logic
  if (collOp < ncclNumFuncs && !base->loggedCollectives[collOp]) {
    for (int q = 0; q < base->nqps; q++) {
      int devIndex = base->qps[q].devIndex;
      int ibDevIndex;

      if (isSend) {
        ibDevIndex = ((struct ncclIbSendComm*)comm)->devs[devIndex].base.ibDevN;
      } else {
        ibDevIndex = ((struct ncclIbRecvComm*)comm)->devs[devIndex].base.ibDevN;
      }

      INFO(NCCL_REG, "%s,CommHash,%lx,Ifname,%s,Ifdev,%d,Collective,%s,QP,%d",
           logPrefix, commHash, ncclIbDevs[ibDevIndex].devName, ncclIbDevs[ibDevIndex].device,
           ncclFuncToString((ncclFunc_t)collOp), base->qps[q].qp->qp_num);
    }
    base->loggedCollectives[collOp] = true;
  }
