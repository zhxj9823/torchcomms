...
ncclResult_t ncclGetLevel(int* level, bool disable, const std::string& levelVal, const char* levelEnv) {
  if (*level == -1) {
    int l = disable ? 0 : -1;
    if (l == -1) {
      if (!levelVal.empty()) {
        for (int i=0; i<=PATH_SYS; i++) {
          if (strcmp(levelVal.c_str(), topoPathTypeStr[i]) == 0) {
            l = i;
            break;
          }
        }
        ...
        if (l == -1 && levelVal.at(0) >= '0' && levelVal.at(0) <= '9') {
          int oldLevel = std::stoi(levelVal);
  ...
  *p2p = 0;
  ...
  if (*p2p == 1) {
    ...
    if (g1 != g2 && (comm == NULL || (info1->hostHash == comm->peerInfo[comm->rank].hostHash &&
                                      info1->hostHash == info2->hostHash)) && NCCL_IGNORE_DISABLED_P2P != 2) {
      int indexes[3] = {-1,-1,-1};
      int verticeN = 0;
...
ncclResult_t ncclTopoCheckGdr(struct ncclTopoSystem* system, int rank, int64_t netId, int read, enum ncclTopoGdrMode* gdrMode) {
  *gdrMode = ncclTopoGdrModeDisable;
  ...
  if (NCCL_NET_GDR_C2C && distance == PATH_P2C) {
...
ncclResult_t ncclTopoNeedFlush(struct ncclComm* comm, int64_t netId, int netDev, int rank, int* flush) {
  *flush = 1;
  ...
  if (props.forceFlush == 1 || NCCL_NET_FORCE_FLUSH) return ncclSuccess;
  int g;
    ...
    for (int g=0; g<system->nodes[GPU].count; g++) {
      ...
      if (ncclPxnDisable(comm) != 1) {
        ...
        if (localGpuIndex != g && localGpuIndex != -1) {
          ...
          int pxnType = NCCL_PXN_C2C ? PATH_P2C : PATH_PXB;
...
ncclResult_t ncclTopoComputeP2pChannels(struct ncclComm* comm) {
  ...
  comm->p2pnChannels = std::min(comm->p2pnChannels, pow2Down(ncclDevMaxChannelsForArgsBytes(NCCL_WORK_ARGS_BYTES)));
