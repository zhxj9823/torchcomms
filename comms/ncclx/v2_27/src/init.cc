...
static void ncclxCommFree(ncclComm_t comm) {
  ...
  if (comm->memCache) {
    comm->memCache.reset();
    ncclx::transport::tranportProxyShutdown(comm);
  }
}
...
ncclResult_t devCommSetup(ncclComm_t comm) {
  ...
  tmpCommAndChans.comm.buffsShared = (comm->memCache != nullptr);

  comm->workArgsBytes = std::min<size_t>(NCCL_WORK_ARGS_BYTES, ncclMaxKernelArgsSize(comm->cudaArch));
  ...
  if (ccEnable) {
    ...
    comm->workFifoBytes = NCCL_WORK_FIFO_BYTES;
  ...
  if (ncclGdrCopy != NULL && NCCL_GDRCOPY_FIFO_ENABLE == 1) {
    ...
    NCCLCHECKGOTO(ncclGdrCudaCalloc(&comm->workFifoBuf, &comm->workFifoBufDev, comm->workFifoBytes, &comm->workFifoBufGdrHandle, comm), ret, fail);
  ...
  // MNNVL support
  {
    ...
    if (info->fabricInfo.state != NVML_GPU_FABRIC_STATE_NOT_SUPPORTED) {
      if (NCCL_MNNVL_UUID != -1) {
        ((long*)&info->fabricInfo.clusterUuid)[0] = NCCL_MNNVL_UUID;
        ((long*)&info->fabricInfo.clusterUuid)[1] = NCCL_MNNVL_UUID;
      }
      if (NCCL_MNNVL_DETERMINISTIC_COLLECTIVE_ENABLE && NCCL_MNNVL_CLIQUE_SIZE <= 0) {
        XCHECK(false) <<
          "NCCL_MNNVL_CLIQUE_SIZE must be set to a positive integer when NCCL_MNNVL_DETERMINISTIC_COLLECTIVE_ENABLE is set";
      }
      if (NCCL_MNNVL_DETERMINISTIC_COLLECTIVE_ENABLE && NCCL_MNNVL_CLIQUE_SIZE > 0) {
        int cliqueId = -1;
        ...
        info->fabricInfo.cliqueId = NCCL_MNNVL_CLIQUE_ID;
      }
...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  else if (comm->isAllNvlink) comm->p2pChunkSize = NCCL_P2P_NVL_CHUNKSIZE;
...
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent, uint64_t timers[TIMERS_INIT_COUNT]) {
  ...
  if ((nNodes > 1 && NCCL_MNNVL_ENABLE != 0 && p2pLevel != 0) || NCCL_MNNVL_ENABLE == 1) {
  ...
  do {
    ...
    for (int i = 0; i < nranks; i++) {
      // (meta-ncclx) Compute min and max compute capabilities
    ...
    // (meta-ncclx) Have NVLS Reg support if there is only one intra process rank
  ...
  if (!NCCL_TOPO_DUMP_FILE.empty()) {
    NCCLCHECKGOTO(ncclTopoGetSystem(comm, NULL, NCCL_TOPO_DUMP_FILE.c_str()), ret, fail);
  }
  ...
  comm->symmetricSupport = comm->isAllDirectP2p && comm->nNodes == 1 && NCCL_WIN_ENABLE && ncclCuMemEnable();
  comm->baseStride = 0;
...
static ncclResult_t ncclCommInitRankFunc(struct ncclAsyncJob* job_) {
  ...
  auto contextMyrank = EventsScubaUtil::StickyContextGuard(ScubaContextKeys::cuda_dev, fmt::format("{}", cudaDev));
  auto contextCudaDev = EventsScubaUtil::StickyContextGuard(ScubaContextKeys::rank, fmt::format("{}", job->myrank));
  auto sampleGuardBegin = EVENTS_SCUBA_UTIL_SAMPLE_GUARD("INIT");
  sampleGuardBegin.sample().setCommunicatorMetadata(comm? &comm->logMetaData: nullptr);
  ...
  // TODO: move CtranComm init here
  // comm->memCache has to be assigned before initTransportsRank
  // to ensure memory allocation there can leverage memory cache.
  if (NCCL_USE_MEM_CACHE) {
    if (!NCCL_LAZY_SETUP_CHANNELS || !NCCL_RUNTIME_CONNECT) {
      WARN("NCCL_USE_MEM_CACHE is assumed to be used for all other NCCL memory optimizations, but NCCL_LAZY_SETUP_CHANNELS=%d and NCCL_RUNTIME_CONNECT=%d", (int)NCCL_LAZY_SETUP_CHANNELS, (int)NCCL_RUNTIME_CONNECT);
      return ncclInvalidUsage;
    }
    comm->memCache = ncclx::memory::memCacheAllocator::getInstance();
  }
  ...
  if (comm->tuner) {
    NCCLCHECK(comm->tuner->init(comm->nRanks, comm->nNodes, ncclMetaDebugLog, &comm->tunerContext));
  }
...
static ncclResult_t envConfigOverride(ncclComm_t comm) {
  ...
  shrinkShareEnv = NCCL_COMM_SHRINK_SHARE_RESOURCES;
  ...
  comm->config.collnetEnable = NCCL_COLLNET_ENABLE;
  INFO(NCCL_ENV, "NCCL_COLLNET_ENABLE set by environment to %d.", NCCL_COLLNET_ENABLE);

  ctaPolicyEnv = NCCL_ENV_CTA_POLICY;
  if (ctaPolicyEnv != NCCL_CONFIG_UNDEF_INT) {
    comm->config.CTAPolicy = ctaPolicyEnv;
  }

  nvlsCTAsEnv = NCCL_NVLS_NCHANNELS;
...
static ncclResult_t commDestroySync(struct ncclAsyncJob* job_) {
  ...
  if (comm->initState == ncclSuccess) {
    ...
    INFO(NCCL_INIT, "Start polling until all graphs referencing us die.");
  ...
  INFO(NCCL_INIT, "Stopping nccl proxy.");
...
ncclResult_t ncclCommAbort(ncclComm_t comm) {
  ...
  // Skip ctranFinalize() to avoid cross-rank sync in abort path
