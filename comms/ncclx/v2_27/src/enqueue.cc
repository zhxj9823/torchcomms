...
static void finishPlan(struct ncclComm* comm, struct ncclKernelPlan* plan) {
  ...
  if (comm->memCache) {
    // Enqueue the request to proxy thread to ensure resources/buffers
    // are ready before communication happens
    NCCLCHECKIGNORE(
        metaCommToNccl(
        comm->transportProxy_->enqueuePrepRequest(
          comm,
          plan->channelMask,
  ...
  while (nPlanColls!=0 && !ncclIntruQueueEmpty(&planner->collTaskQueue)) {
    ...
    if (task->isCollnet) {
      ...
      for (int c=devWork->channelLo; c <= (int)devWork->channelHi; c++) {
        ...
        NCCLCHECK(addProfilerProxyOpIfNeeded(comm, plan, proxyOp));
        if (comm->memCache) {
          NCCLCHECK(ncclx::addCollBufKeysToKernelPlan(comm, c, task, plan));
        }
      }
    }
...
static ncclResult_t addP2pToPlan(
  ...
  ssize_t thresholdLL = nChannelsMax*NCCL_P2P_LL_THRESHOLD;
  ssize_t paramChunkSize = NCCL_CHUNK_SIZE;
  ...
  for (int part=0; part < nChannelsMax; part++) {
    ...
    for (int dir=0; dir < nProxyOps; dir++) {
      ...
      if (nParts <= part) {
        ...
        if (comm->memCache) {
          NCCLCHECK(ncclx::addP2PBufKeysToKernelPlan(
            comm,
            /*isSend=*/dir == 1,
            channelId,
            connIndex,
            (dir ? sendRank : recvRank),
            plan));
        }
      }
...
ncclResult_t ncclLaunchPrepare(struct ncclComm* comm) {
  ...
  if (planner->nTasksColl + planner->nTasksP2p != 0) {
    do {
      ...
      if (comm->memCache) {
        plan->bufKeys.clear();
        // Max buffer usage in a plan would be:
        // (number of initialized channels) * (number of peers for collective and send/recv) * (send + recv)
        int nChannels = (NCCL_LAZY_SETUP_CHANNELS) ? comm->nChannelsReady : comm->nChannels;
        int nPeers = ((planner->nTasksColl > 0) ? NCCL_MAX_TREE_ARITY : 0) + ((planner->nTasksP2p > 0) ? comm->nRanks : 0);
...
ncclResult_t ncclLaunchKernel(struct ncclComm* comm, struct ncclKernelPlan* plan) {
  ...
  if (CUDART_VERSION >= 11080 && driverVersion >= 11080) {
    ...
    if (compCap >= 90 && driverVersion >= 12000) {
      ...
      launchAttrs[attrs++].value.memSyncDomain = (CUlaunchMemSyncDomain) NCCL_MEM_SYNC_DOMAIN;
    }
    #endif
    ...
    if (compCap >= 90 && driverVersion >= 13000) {
      ...
      launchAttrs[attrs].value.nvlinkUtilCentricScheduling = NCCL_NVLINK_UTIL_CENTRIC_SCHED_ENABLE;
      attrs++;
    }
    #endif
...
static ncclResult_t topoGetAlgoInfo(
  ...
  if (info->algorithm == NCCL_ALGO_UNDEF || info->protocol == NCCL_PROTO_UNDEF) {
    ...
    if (NCCL_ALGO.size()) {
      snprintf(ncclAlgoEnvStr, 1023, " NCCL_ALGO was set to %s.", NCCL_ALGO.c_str());
    }
    if (NCCL_PROTO.size()) {
      snprintf(ncclProtoEnvStr, 1023, " NCCL_PROTO was set to %s.", NCCL_PROTO.c_str());
    }
    ...
    return (NCCL_ALGO.size() || NCCL_PROTO.size()) ? ncclInvalidUsage : ncclInternalError;
  }
...
static ncclResult_t getAlgoInfo(
  ...
  if (comm->tuner != NULL) {
    ...
    regBuff = (regSendBuf && regRecvBuf && isSendValid && isRecvValid) || (ncclCudaGraphValid(comm->planner.capturingGraph) && NCCL_GRAPH_REGISTER);
    ...
    if (comm->config.CTAPolicy == NCCL_CTA_POLICY_EFFICIENCY && NCCL_ALGO.empty() && NCCL_PROTO.empty() && !comm->MNNVL) {
...
static ncclResult_t calcCollChunking(
  ...
  if (info->algorithm == NCCL_ALGO_COLLNET_DIRECT) {
    ...
    int maxChunkSize = (int)NCCL_NVLSTREE_MAX_CHUNKSIZE;
...
static ncclResult_t hostToDevRedOp(
  ...
  case ncclAvg:
    ...
    case ncclInt8:  case ncclInt32:  case ncclInt64:
      ...
      [[fallthrough]]; // Intentional fall-through to RAS_CLIENT_COMMS
